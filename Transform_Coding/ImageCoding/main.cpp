...
char* g_windowName = "Transform-Coding-Image";

#define IMAGE_FILE "data/cameraman.ppm" //File to transform
		...
		for ( int j = 0; j < 8; j++)
		{
			...
			if (i + j <= m)
			{
				for (int x = 0; x < 8; x++)
				{
					for (int y = 0; y < 8; y++)
					{
						cMatrixSum += A[x * 8 + y] * cos((M_PI*i*(2 * x + 1)) / 16)*cos((M_PI*j*(2 * y + 1)) / 16);
					}
				}
			}
		...
		for (int y = 0; y < 8; y++)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				for (int j = 0; j < 8; j++)
				{
					...
					bMatrixSum += cU * cV * C[i][j] * cos((M_PI*i*(2 * x + 1)) / 16) * cos((M_PI*j*(2 * y + 1)) / 16);
				}
			}
			B[x*g_image_width + y] = bMatrixSum;
		}
	}
}
...
void CompressImage(const std::vector<float> I, std::vector<float>& O, int m)
{
	for (int i = 0; i < g_image_height / 8; i++)
	{
		for (int j = 0; j < g_image_width / 8; j++)
		{
			...
			int blockLocation = i * g_image_width * 8 + j * 8;
			for (int x = 0; x < 8; x++)
			{
				for (int y = 0; y < 8; y++)
				{
					block[x * 8 + y] = I[blockLocation + (x * g_image_width + y)];
				}
			}
			CompressBlock(block, &O[blockLocation], m);
		}
	}
}
...
int main()
{
	...
	int n = 1;	//change the parameter n from 1 to 16 to see different image quality
