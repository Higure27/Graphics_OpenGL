...
void computeJointTransformations(
	...
	const std::vector<int>& p_jointParent,//p(j) 
...
void skinning(
		const std::vector<Vector3f>& p_vertices,		//rest pose vertex positions
														//
		const unsigned int p_numJoints,					//number of joints
														//
		const std::vector<Matrix4f>& p_jointTrans,		//the transformations 
														//(p_global)you have computed
														//in the computeJointTransformations
														//
		const std::vector<Matrix4f>& p_jointTransRestInv,//concatenated joint transformations
														//computed at
														//the rest pose and inverted
														//
		const std::vector<std::vector<float>>& p_weights,// a 2D array of size 
														 //numJoints x numVertices
														//
		std::vector<Vector3f>& p_deformedVertices)		//deformed vertices computed 
														//by linear blend skinning
{
	for (unsigned int v = 0; v < p_vertices.size(); v++)
	{	

		Vector4f homogV = toHomog(p_vertices[v]);
		Vector4f homogVfinal(4);
		homogVfinal.fill(0);

		for (unsigned int i = 0; i < p_numJoints; i++)
		{
			homogVfinal += p_weights[i][v]*p_jointTrans[i]* p_jointTransRestInv[i]*homogV;
		} 

		p_deformedVertices[v] = fromHomog(homogVfinal);
	}
}
...
void initRestPose()
{
	...
	for (unsigned int jointID = 0; jointID < g_numJoints; ++jointID)
	{
		g_jointTransRestInv[jointID] = g_jointTrans[jointID].inverse();//computes A_-1
	}
}
...
{
	//loadData("capsule"); // replace this with the following line to load the capsule instead
	loadData("ogre");
